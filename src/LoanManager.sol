...
/**
 * @title LoanManager
 * @notice Core orchestrator for Invalend PoC (20% margin, 80% pool funding)
...
contract LoanManager is ReentrancyGuard {
    ...
    // ----------- Events -----------
    event LoanCreated(address indexed borrower, uint256 loanAmount, uint256 marginAmount, uint256 poolFunding, address indexed restrictedWallet);
    ...
    function repayLoan() external nonReentrant returns (bool success) {
        ...
        if (returnedAmount >= totalRepayment) {
            ...
            userProfit = loan.marginAmount > (totalRepayment - returnedAmount) ? loan.marginAmount - (totalRepayment - returnedAmount) : 0;
        }
        if (poolRecovery > 0) {
